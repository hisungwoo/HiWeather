...
//    Log.e(">>", "x = " + tmp.x + ", y = " + tmp.y);
    ...
    public LatXLngY convertGRID_GPS(int mode, double lat_X, double lng_Y )
    {
        ...
        if (mode == TO_GRID) {
            ...
            Double test = Math.floor(ra * Math.sin(theta) + XO + 0.5);

            rs.x = String.format(Locale.getDefault(), "%.0f" , test);
            rs.y = String.format(Locale.getDefault(), "%.0f" , Math.floor(ro - ra * Math.cos(theta) + YO + 0.5));
        }
